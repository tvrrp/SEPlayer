...
class PlayerViewControllerSim: UIViewController {
    let player1: Player
    let player2: Player
    let player3: Player
    let player4: Player
    ...
    let playerView1 = SEPlayerView()
    let playerView2 = SEPlayerView()
    let playerView3 = SEPlayerView()
    let playerView4 = SEPlayerView()
    ...
    init() {
        ...
        player4 = playerFactory.buildPlayer()
    ...
    override func viewDidLayoutSubviews() {
        ...
        playerView1.frame = CGRect(x: 0, y: 0, width: view.bounds.width / 2, height: view.bounds.height / 2)
        playerView2.frame = CGRect(x: view.bounds.width / 2, y: 0, width: view.bounds.width / 2, height: view.bounds.height / 2)
        playerView3.frame = CGRect(x: 0, y: view.bounds.height / 2, width: view.bounds.width / 2, height: view.bounds.height / 2)
        playerView4.frame = CGRect(x: view.bounds.width / 2, y: view.bounds.height / 2, width: view.bounds.width / 2, height: view.bounds.height / 2)
    }
...
//        player.release()
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.insertSubview(playerView1, at: 0)
        view.insertSubview(playerView2, at: 1)
        view.insertSubview(playerView3, at: 2)
        view.insertSubview(playerView4, at: 3)
...
//        player.playWhenReady = true
        let urls = [
            URL(string: "https://storage.googleapis.com/exoplayer-test-media-0/shorts_android_developers/shorts_11.mp4")!,
            URL(string: "https://storage.googleapis.com/exoplayer-test-media-0/shorts_android_developers/shorts_9.mp4")!,
            URL(string: "https://storage.googleapis.com/exoplayer-test-media-0/shorts_android_developers/shorts_3.mp4")!,
    ...
    @objc func handleLongPress(_ sender: UILongPressGestureRecognizer) {
        ...
        case .began:
            ...
            if location.x < 50 || location.x > (view.bounds.width - 50) {
                ...
                players.forEach { $0.playbackParameters = PlaybackParameters(playbackRate: 2.0) }
            }
        case .ended, .cancelled, .failed:
            players.forEach { $0.playbackParameters = PlaybackParameters(playbackRate: 1) }
        default:
            return
        }
    }
...
//        player.seekBack()
        players.forEach { $0.seekBack() }
        handleTimer()
    }

    @objc func forwardsTap() {
//        player.seekForward()
        players.forEach { $0.seekForward() }
        handleTimer()
    }
...
extension PlayerViewControllerSim: SEPlayerDelegate {
