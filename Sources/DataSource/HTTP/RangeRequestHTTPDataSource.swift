...
final class RangeRequestHTTPDataSource: DataSource {
    ...
    func close(isolation: isolated any Actor) async throws -> ByteBuffer? {
        syncActor.assertIsolated()
        let result = try await requestHandler?.close()
        requestHandler = nil
        return result
    }
    ...
    func read(to buffer: inout ByteBuffer, offset: Int, length: Int, isolation: isolated any Actor) async throws -> DataReaderReadResult {
        syncActor.assertIsolated()
        guard let requestHandler, let currentDataSpec, let bytesRemaining else { throw DataSourceError(reason: .connectionNotOpened) }
    ...
    private func openConnection(dataSpec: DataSpec, isolation: isolated any Actor) async throws -> Int {
        ...
        guard let urlResponse else { throw URLSessionDataSourceError(failedURL: currentDataSpec?.url, errorReason: .badServerResponse) }
