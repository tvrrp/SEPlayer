...
final class ATRenderer: BaseSERenderer {
    ...
    private let audioSync: IAudioSync
    ...
    init(
        ...
        audioSync = try AudioSync(queue: queue, clock: clock)
        ...
        do {
            let result = try sampleBuffer.withUnsafeAudioStreamPacketDescriptions { description in
                return try sampleBuffer.withAudioBufferList { audioBuffer, _ in
                    ...
                    dataObject.packetPointer?.deallocate()
                    return result
                }
            }
...
private struct DataObject {
    ...
    var packetPointer: UnsafeMutablePointer<AudioStreamPacketDescription>?
...
private func converterComplexBufferCallback(
    ...
    packetDescPointer.initialize(to: dataObject.pointee.packetDescription[0])
    outDataPacketDescription?.pointee = packetDescPointer
    dataObject.pointee.packetPointer = packetDescPointer
