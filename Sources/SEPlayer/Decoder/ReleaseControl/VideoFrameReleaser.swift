...
protocol SampleReleaser {
    var isReady: Bool { get }
    func dequeueFirstSampleIfNeeded()
}

final class VideoFrameReleaser: SampleReleaser {
...
//            }
            if videoRenderer.hasSufficientMediaDataForReliablePlaybackStart {
                needsFirstDequeue = false
                try? timebase.setTimerToFireImmediately(timer)
            }
        }
...
//        guard !videoRenderer.hasSufficientMediaDataForReliablePlaybackStart else {
//            return false
//        }
...
//            print("😎 videoRenderer.isReady = \(videoRenderer.hasSufficientMediaDataForReliablePlaybackStart), currentSample = \(currentSample)")
            currentSample += 1
            return videoRenderer.hasSufficientMediaDataForReliablePlaybackStart ? true : automaticDequeue()
        } else {
            return false
        }
    }
...
private extension VideoFrameReleaser {
