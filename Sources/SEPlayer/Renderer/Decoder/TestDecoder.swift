...
protocol SimpleDecoderInputBuffer: AnyObject {
...
protocol SimpleDecoderOutputBuffer: AnyObject {
    var flags: SampleFlags { get set }
    var timeUs: Int64 { get set }
    var shouldBeSkipped: Bool { get set }
    var skippedOutputBufferCount: Int { get set }

    func release()
    func clear()
}

class SimpleDecoder<I: SimpleDecoderInputBuffer, O: SimpleDecoderOutputBuffer, E: Error>: Decoder {
    ...
    func releaseOutputBuffer(_ outputBuffer: O, isolation: isolated PlayerActor = #isolation) {
        lock.withLock {
    ...
    func flush() {
        lock.withLock {
            ...
            queuedInputBuffers.removeAll(where: {
                releaseInputBufferInternal($0)
                return true
            })
            queuedOutputBuffers.removeAll(where: {
                $0.release(); return true
            })
        }
    }
