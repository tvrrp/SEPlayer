...
public final class Message {
    public var what: MessageKind = EmptyMessageKind()
    public var inUse = false
    public var whenNs: UInt64 = .zero
    ...
    private nonisolated(unsafe) static var pool: Message?
    private nonisolated(unsafe) static var poolSize: Int = .zero
    ...
    public static func obtain(
        ...
        let msg = obtain()
        msg.target = handler
        msg.what = what ?? EmptyMessageKind()
        return msg
    }

    public static func obtain() -> Message {
        lock.lock(); defer { lock.unlock() }

        if let pool = pool {
            let message = pool
            self.pool = message.next
            message.next = nil
            message.inUse = false
            poolSize -= 1
            return message
