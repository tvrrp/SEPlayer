...
public final class MessageQueue: @unchecked Sendable {
    public protocol IdleHandler: AnyObject {
        nonisolated func queueIdle() -> Bool
    }
    ...
    public func addIdleHandler(_ handler: IdleHandler) {
        lock.withLock { idleHandlers.add(handler) }
    }

    public func removeIdleHandler(_ handler: IdleHandler) {
        lock.withLock { idleHandlers.remove(handler) }
    }
    ...
    public func hasMessages(handler: Handler, what: MessageKind) -> Bool {
        lock.withLock {
            var p = messages
            while let previous = p {
    ...
    public func removeMessages(handler: Handler, what: MessageKind) {
        lock.withLock {
            var p = messages
